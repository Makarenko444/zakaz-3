...
- `POST /api/admin/address-backup-import` - импорт оригинальных адресов из TSV
- `GET /api/admin/address-backup-import` - статистика заполненности оригинальных адресов
- `DELETE /api/admin/address-backup-import` - очистка полей оригинальных адресов
...
#### Нормализация адресов (в процессе)
- ✅ Поле города (city) для поддержки Томск/Северск
- ✅ Резервное копирование оригинальных адресов (street_and_house_original, address_details_original)
- ✅ Импорт оригинальных адресов из TSV через админ-панель
- ✅ Поиск по второму адресному полю (address_details)
- ✅ Функциональный поиск в шапке сайта
- 🔄 Нормализация данных (1990 → 1973 дубликатов)
...
node_id           UUID REFERENCES zakaz_nodes(id)
city              TEXT NOT NULL DEFAULT 'Томск'
street_and_house  TEXT                          -- Нормализованный адрес (улица и дом)
address_details   TEXT                          -- Детали адреса (квартира, этаж, офис)
street_and_house_original TEXT                  -- Оригинальный адрес (для сравнения)
address_details_original  TEXT                  -- Оригинальные детали адреса
address_match_status TEXT DEFAULT 'unmatched'   -- Статус привязки к справочнику
...
created_at        TIMESTAMPTZ DEFAULT NOW()
updated_at        TIMESTAMPTZ DEFAULT NOW()
application_number SERIAL                       -- Порядковый номер заявки
legacy_id         INTEGER                       -- ID из старой системы (для импорта)
```

**Особенности:**
- Поля `street_and_house_original` и `address_details_original` хранят исходные данные из импорта для сравнения с нормализованными значениями
- Поле `city` добавлено для поддержки нескольких городов (Томск, Северск)
- `address_match_status` отслеживает привязку к справочнику адресов

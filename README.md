# Zakaz-3 - Система управления заявками

Веб-приложение для управления заявками на основе Next.js 15 и Supabase.

## 🌐 Серверы

**Production (боевой):**
- URL: https://zakaz2.tomica.ru
- Сервер: zakaz2.tomica.ru (78.140.57.33)
- Директория: `~/projects/zakaz-2`
- PM2 процесс: `zakaz-2`

**Development (тестовый):**
- URL: https://zakaz3.tomica.ru
- Сервер: zakaz3.tomica.ru (78.140.55.90)
- Директория: `~/projects/zakaz-3`
- PM2 процесс: `zakaz-3`

## 📋 Описание

Система для управления заявками с поддержкой различных ролей пользователей, отслеживанием статусов, комментариями и полным аудитом действий.

## 🚀 Основные возможности

- **Аутентификация и авторизация** - 10 ролей пользователей с разными уровнями доступа
- **Управление заявками** - создание, редактирование, просмотр, фильтрация
- **Статусы заявок** - справочник статусов из БД с историей изменений
- **Назначение исполнителей** - привязка заявок к пользователям
- **Наряды на работы** - создание нарядов на осмотр и монтаж, назначение исполнителей, отслеживание статусов
- **Календарь планирования** - визуализация нарядов по дням и неделям, загрузка сотрудников
- **Комментарии** - система комментариев к заявкам с возможностью ответа и прикрепления файлов
- **Управление файлами** - просмотр всех файлов, статистика по типам, миниатюры изображений, фильтрация
- **Управление узлами подключения** - импорт из Excel, фильтрация, поиск, просмотр и редактирование узлов сети (АО, ПРП, СК, РТК)
- **Аудит действий** - полное логирование всех действий в системе
- **Адресная система** - управление адресами объектов, поддержка нескольких городов (Томск, Северск)
- **Нормализация адресов** - импорт оригинальных адресов, инструменты для нормализации и устранения дубликатов

## 🛠️ Технологии

- **Frontend**: Next.js 15 (App Router), React 19, TypeScript, TailwindCSS 4
- **Backend**: Next.js API Routes
- **База данных**: Supabase (PostgreSQL)
- **Валидация**: Zod + React Hook Form
- **Аутентификация**: Supabase Auth

## 📦 Установка

### 1. Клонирование репозитория

```bash
git clone https://github.com/Makarenko444/zakaz-3.git
cd zakaz-3
```

### 2. Установка зависимостей

```bash
npm install
```

### 3. Настройка переменных окружения

Скопируйте `.env.example` в `.env.local` и заполните значения:

```bash
cp .env.example .env.local
```

Отредактируйте `.env.local`:

```env
# Public Supabase URL (через Nginx reverse proxy для HTTPS)
NEXT_PUBLIC_SUPABASE_URL=https://your-domain.com/api

# Public Supabase Anon Key
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_anon_key_here

# Private Supabase Service Role Key (используется только на сервере)
SUPABASE_SERVICE_ROLE_KEY=your_service_role_key_here

# Direct Supabase URL (для server-side операций, минуя Nginx)
SUPABASE_DIRECT_URL=http://your-supabase-ip:8000
```

### 4. Настройка базы данных

**Базовые таблицы должны быть уже созданы** (`zakaz_users`, `zakaz_applications`, `zakaz_addresses` и т.д.)

Если требуется, выполните дополнительные миграции в Supabase Dashboard в порядке:

```bash
# Дополнительные таблицы (выполняются в Supabase Dashboard -> SQL Editor)
database/migrations/005_create_audit_log.sql              # Журнал аудита действий
database/migrations/006_create_comments.sql               # Комментарии к заявкам
database/migrations/007_create_application_statuses.sql   # Справочник статусов
database/migrations/016_add_reply_to_comments.sql         # Ответы на комментарии
database/migrations/018_create_nodes_table.sql            # Таблица узлов подключения
database/migrations/019_fix_node_types.sql                # Исправление типов узлов
database/migrations/029_add_city_field.sql                # Поле город в заявках
database/migrations/030_add_original_address_fields.sql   # Поля для оригинальных адресов
```

**Примечание**: Если базовые таблицы еще не созданы, создайте их согласно техническому заданию или обратитесь к администратору БД.

## 🏃 Запуск

### Режим разработки

```bash
npm run dev
```

Откройте [http://localhost:3000](http://localhost:3000) в браузере.

### Production сборка

```bash
# Собрать приложение
npm run build

# Запустить production сервер
npm run start
```

### Проверка кода

```bash
npm run lint
```

## 📁 Структура проекта

```
zakaz-3/
├── app/                          # Next.js App Router
│   ├── api/                      # API Routes
│   │   ├── applications/         # API заявок
│   │   ├── addresses/            # API адресов
│   │   ├── files/                # API файлов и статистики
│   │   ├── nodes/                # API узлов подключения
│   │   ├── work-orders/          # API нарядов
│   │   ├── schedule/             # API календаря
│   │   ├── statuses/             # API статусов
│   │   └── users/                # API пользователей
│   ├── components/               # React компоненты
│   ├── dashboard/                # Страницы дашборда
│   │   ├── applications/         # Страницы заявок
│   │   ├── work-orders/          # Страницы нарядов
│   │   ├── schedule/             # Календарь планирования
│   │   ├── files/                # Управление файлами
│   │   └── nodes/                # Управление узлами подключения
│   ├── print/                    # Печатные формы
│   │   └── work-orders/          # Печать нарядов
│   ├── login/                    # Страница входа
│   └── layout.tsx                # Корневой layout
├── lib/                          # Утилиты и библиотеки
│   ├── supabase.ts               # Supabase клиент (браузер)
│   ├── supabase-direct.ts        # Прямое подключение (сервер)
│   ├── audit-log.ts              # Система аудита
│   ├── auth-client.ts            # Клиентская аутентификация
│   ├── session.ts                # Управление сессиями
│   └── types.ts                  # TypeScript типы
├── database/                     # SQL миграции
│   └── migrations/
├── docs/                         # Документация
│   ├── progress/                 # История разработки
│   ├── workflow.md               # Схема работы над проектом
│   └── DEPLOYMENT.md             # Инструкции по развертыванию
├── frontend/                     # Стили и ресурсы
├── middleware.ts                 # Next.js middleware
└── package.json
```

## 👥 Роли пользователей

| Роль | Код | Описание |
|------|-----|----------|
| Администратор | `admin` | Полный доступ ко всем функциям |
| Менеджер | `manager` | Управление заявками, назначение исполнителей |
| Инженер | `engineer` | Техническая работа с заявками |
| Монтажник | `installer` | Выполнение монтажных работ |
| Снабжение | `supply` | Работа с материалами и поставками |
| Директор | `director` | Руководитель |
| Бухгалтер | `accountant` | Финансовый учет |
| Тех.поддержка | `support` | Техническая поддержка клиентов |
| Эксплуатация | `maintenance` | Эксплуатационный персонал |
| Согласование | `approval` | Согласование документов |

## 🔐 Аутентификация

Используется Supabase Auth с проверкой на middleware уровне. Все страницы кроме `/login` требуют аутентификации.

## 📊 База данных

### Основные таблицы:

- `zakaz_users` - пользователи системы
- `zakaz_applications` - заявки (включая поля `city`, `street_and_house`, `address_details`, `street_and_house_original`, `address_details_original`)
- `zakaz_addresses` - адреса объектов
- `zakaz_nodes` - узлы подключения (АО, ПРП, СК, РТК)
- `zakaz_application_statuses` - справочник статусов
- `zakaz_application_status_history` - история изменения статусов
- `zakaz_application_comments` - комментарии к заявкам
- `zakaz_files` - прикрепленные файлы к заявкам и комментариям
- `zakaz_sessions` - пользовательские сессии
- `zakaz_audit_log` - журнал аудита действий

### Таблицы нарядов:

- `zakaz_work_orders` - наряды на работы (осмотр, монтаж)
- `zakaz_work_order_executors` - исполнители нарядов (many-to-many)
- `zakaz_work_order_materials` - материалы для выполнения работ
- `zakaz_work_order_status_history` - история изменений статусов нарядов

### Поля адресов в заявках:

| Поле | Описание |
|------|----------|
| `city` | Город (Томск, Северск) |
| `street_and_house` | Нормализованный адрес (улица и дом) |
| `address_details` | Детали адреса (квартира, этаж, офис) |
| `street_and_house_original` | Оригинальный адрес из импорта (для сравнения) |
| `address_details_original` | Оригинальные детали из импорта |

## 🔄 Workflow деплоя

### Быстрый деплой (рекомендуется)

Используйте готовые скрипты для автоматического развертывания:

**На production сервер (zakaz2.tomica.ru):**
```bash
# Подключитесь к серверу
ssh user@zakaz2.tomica.ru

# Запустите деплой
pr-zakaz2
```

**На development сервер (zakaz3.tomica.ru):**
```bash
# Подключитесь к серверу
ssh user@zakaz3.tomica.ru

# Запустите деплой
pr-zakaz3
```

Скрипты автоматически выполняют:
1. ✅ Переход в директорию проекта
2. ✅ Git pull последних изменений
3. ✅ Установку зависимостей
4. ✅ Очистку старого билда
5. ✅ Сборку проекта
6. ✅ Перезапуск PM2 процесса

### Ручной деплой

Если нужно выполнить деплой вручную:

**Production (zakaz2.tomica.ru):**
```bash
cd ~/projects/zakaz-2
git pull origin main
npm install
rm -rf .next
npm run build
pm2 restart zakaz-2
```

**Development (zakaz3.tomica.ru):**
```bash
cd ~/projects/zakaz-3
git pull origin main
npm install
rm -rf .next
npm run build
pm2 restart zakaz-3
```

### Настройка алиасов (первый раз)

Если алиасы еще не настроены, выполните на сервере:

```bash
# Для zakaz2
echo "alias pr-zakaz2='cd ~/projects/zakaz-2 && ./pr-zakaz2.sh'" >> ~/.bashrc
source ~/.bashrc

# Для zakaz3
echo "alias pr-zakaz3='cd ~/projects/zakaz-3 && ./pr-zakaz3.sh'" >> ~/.bashrc
source ~/.bashrc
```

Подробные инструкции см. в [DEPLOY_SCRIPTS_README.md](./DEPLOY_SCRIPTS_README.md)

## 📝 Разработка

### Создание новой функциональности:

1. Создайте feature ветку
2. Внесите изменения
3. Протестируйте локально
4. Создайте Pull Request в `main`
5. После мержа деплойте на production

### Важно:

- ❌ **НЕ делайте коммиты напрямую на боевом сервере**
- ✅ **Вся разработка через GitHub**
- ✅ **Боевой сервер только для деплоя**

## 🔒 Безопасность

### Скрипт проверки безопасности

В проекте есть скрипт автоматической проверки безопасности сервера:

```bash
./scripts/security-check.sh
```

**Проверяет:**
- Нагрузку системы
- Криптомайнеры и вредоносные процессы
- Подозрительные файлы в /tmp и ~/.local/share
- Инъекции в bashrc/profile
- Crontab записи
- PM2 процессы
- Docker контейнеры
- NPM уязвимости

**Рекомендуется запускать:**
- После каждого деплоя
- При обнаружении высокой нагрузки
- Регулярно (раз в неделю)

### Защита от брутфорса

На серверах установлен **fail2ban** для защиты от SSH брутфорса:

```bash
# Проверка статуса
sudo fail2ban-client status sshd

# Просмотр заблокированных IP
sudo fail2ban-client status sshd | grep "Banned IP"
```

### Важно

- Регулярно обновляйте Next.js: `npm update next`
- Проверяйте уязвимости: `npm audit`
- Следите за нагрузкой: `uptime`

## 📚 Документация

- [Техническая спецификация](./TECHNICAL_SPECIFICATION.md)
- [История разработки](./docs/progress/)
- [Наряды и календарь (16.12.2025)](./docs/progress/2025-12-16-work-orders-and-calendar.md)
- [Нормализация адресов (10.12.2025)](./docs/progress/2025-12-10-address-normalization-and-backup.md)
- [Импорт адресов](./scripts/README_addresses_import.md)

## 🐛 Отладка

### Проблемы с типами TypeScript

Если возникают ошибки типизации Supabase, используйте явное приведение типов:

```typescript
const { data, error } = await supabase
  .from('table_name')
  .select('*')
  .single() as { data: YourType | null; error: unknown }
```

### Проблемы с production билдом

Проверьте:
1. Правильность переменных окружения в `.env.local`
2. Доступность Supabase по указанным URL
3. Логи в консоли браузера и терминала

## 📄 Лицензия

Частный проект

## 👨‍💻 Автор

Makarenko444

---

**Поддержка**: Для вопросов и багов создавайте Issues в GitHub.
